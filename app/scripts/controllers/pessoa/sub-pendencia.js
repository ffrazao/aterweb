...
aterwebApp.controller('PessoaPendenciaCtrl', ['$scope', 'FrzNavegadorParams', '$modal_b', '$modalInstance', 'toastr',
  ...
  $scope.pessoaPendenciaNvg = new FrzNavegadorParams();
  ...
  $scope.especial = function () {
	$scope.pessoaPendenciaNvg.especialBotoesVisiveis(['agir', 'editar', 'excluir', 'incluir', 'navegar', 'tamanhoPagina', ]);
  };
  ...
  $scope.incluir = function (size) {
    var modalInstance = $modal_b.open({
      ...
      templateUrl: 'pessoaPendenciaFrm.html',
      controller: 'PessoaPendenciaCtrl',
      size: size,
      resolve: {
        registro: function () {
    ...
    modalInstance.result.then(function (registro) {
      ...
      if (!$scope.cadastro.registro.pendencia) {
        $scope.cadastro.registro.pendencia = [];
      }
    	if (angular.isArray(registro)) {
    		for (var r in registro) {
    			$scope.cadastro.registro.pendencia.push(r);
    		}
    	} else {
    		$scope.cadastro.registro.pendencia.push(registro);
    	}
    }, function () {
  ...
  $scope.pesquisaPessoa = function(size) {
    ...
    modalInstance.result.then(function (registro) {
      ...
      if (angular.isArray(registro)) {
        $scope.pendencia.pessoa = angular.copy(registro[0]);
      } else {
        $scope.pendencia.pessoa = angular.copy(registro);
      }
    }, function () {
      console.log('Modal dismissed at: ' + new Date());
    });
  ...
  $scope.ok = function () {
  	$modalInstance.close($scope.pendencia);
  };
